...
void xSepia_STD::testCopyContent(xPic &Dst, const xPic &Src)
{
  ...
  // WARNING - for demonstration purposes only - it isn't the fastest way to
  // copy data
...
void xSepia_STD::testYUVtoRGBtoYUV_FLT(xPic &Dst, const xPic &Src, eMode ProcMode)
{
  ...
  case eMode::PIXELS:
    for (int32 y = 0; y < Height; y++)
    {
      for (int32 x = 0; x < Width; x++)
      {
        m_ThreadPool->addWaitingTask([&Dst, &Src, x,
                                      y](int32 /*ThreadIdx*/)
  ...
  case eMode::ROWS:
    for (int32 y = 0; y < Height; y++)
    {
      m_ThreadPool->addWaitingTask(
          [&Dst, &Src, Width, y](int32 /*ThreadIdx*/)
          {
  ...
  case eMode::TILES1024:
  {
    ...
    m_TasksNum = 0;
    applyTiles(Dst, Src, 0, Height, 0, Width, TileSize, Width, Height,
               false);
    m_ThreadPool->waitUntilTasksFinished(m_TasksNum);
...
void xSepia_STD::xTestYUVtoRGBtoYUV_FLT(xPic &Dst, const xPic &Src, int32 BegY, int32 EndY, int32 BegX, int32 EndX)
{
  ...
  for (int32 y = BegY; y < EndY; y++)
  {
    ...
    {
      // load
      const uint16V4 SrcPixelYUV_U16 = {SrcPtrLm[x], SrcPtrCb[x],
                                        SrcPtrCr[x], 0};

      // change data format (and remove chroma offset)
      const flt32V4 SrcPixelYUV_F32 =
          ((flt32V4)SrcPixelYUV_U16 -
           flt32V4(0, SrcMidValue, SrcMidValue, 0));
      ...
      const flt32V4 DstPixelYUV_F32 = {
          DstPixelRGB_F32[0] * RGB2YCbCr_F32[0][0] +
              DstPixelRGB_F32[1] * RGB2YCbCr_F32[0][1] +
              DstPixelRGB_F32[2] * RGB2YCbCr_F32[0][2],
          DstPixelRGB_F32[0] * RGB2YCbCr_F32[1][0] +
              DstPixelRGB_F32[1] * RGB2YCbCr_F32[1][1] +
              DstPixelRGB_F32[2] * RGB2YCbCr_F32[1][2],
          DstPixelRGB_F32[0] * RGB2YCbCr_F32[2][0] +
              DstPixelRGB_F32[1] * RGB2YCbCr_F32[2][1] +
              DstPixelRGB_F32[2] * RGB2YCbCr_F32[2][2],
          0};

      // change data format (and apply chroma offset) + clip to range
      // 0-Max
      flt32V4 TmpPixelYUV_F32 =
          DstPixelYUV_F32 + flt32V4(0, DstMidValue, DstMidValue, 0);
      TmpPixelYUV_F32.clip(xMakeVec4<flt32>(0),
                           xMakeVec4<flt32>(DstMaxValue));
  ...
  for (int32 y = BegY; y < EndY; y++)
  {
    for (int32 x = BegX; x < EndX; x++)
    {
      ...
      const flt32V4 DstPixelRGB_F32 = {
          SrcPixelRGB_F32[0] * SEPIA_F32[0][0] +
              SrcPixelRGB_F32[1] * SEPIA_F32[0][1] +
              SrcPixelRGB_F32[2] * SEPIA_F32[0][2],
          SrcPixelRGB_F32[0] * SEPIA_F32[1][0] +
              SrcPixelRGB_F32[1] * SEPIA_F32[1][1] +
              SrcPixelRGB_F32[2] * SEPIA_F32[1][2],
          SrcPixelRGB_F32[0] * SEPIA_F32[2][0] +
              SrcPixelRGB_F32[1] * SEPIA_F32[2][1] +
...
void xSepia_STD::applyTiles(xPic &Dst, const xPic &Src, int32 BegY, int32 EndY, int32 BegX, int32 EndX, int32 TileSize, int32 Width, int32 Height, bool Sepia)
{
  int32 diffY = EndY - BegY;
  int32 diffX = EndX - BegX;
  ...
  if (!(diffY == 0 || diffX == 0))
  {
    ...
    // Tile downsizing
    while ((TileSize > diffY) || (TileSize > diffX))
    {
      TileSize /= 2;
    }

    for (y = BegY; y <= EndY - TileSize; y += TileSize)
    {
      for (x = BegX; x <= EndX - TileSize; x += TileSize)
      {
        // Here is the "complicated" lambda expression, I have also thrown if statement there to differentiate "sepia" operations
        m_ThreadPool->addWaitingTask(
            [&Dst, &Src, x, y, TileSize, Sepia](int32 /*ThreadIdx*/)
            {
              if (Sepia)
                xApplySepiaEffect_FLT(Dst, Src, y, y + TileSize, x, x + TileSize);
              else
                xTestYUVtoRGBtoYUV_FLT(Dst, Src, y, y + TileSize, x, x + TileSize);
            });
    ...
    // Two additional function calls for remaining parts
    applyTiles(Dst, Src, BegY, y, x, EndX, TileSize, Width, Height, Sepia);
    applyTiles(Dst, Src, y, EndY, BegX, EndX, TileSize, Width, Height, Sepia);
  }
}