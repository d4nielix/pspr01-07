...
int32_t Example02()
{
  ...
  while (1)
  {
    ...
    {
      fmt::print("MAIN pretend to do some work here...\n");
    }
    else
    {
      break;
...
int32_t ThreadFunctionWithArgs(int32_t WaitMilliSeconds)
{
  ...
  fmt::print("THRD ID={} FINISH\n", ThreadID);
  int32_t RetVal = WaitMilliSeconds;
  return RetVal;
}
...
int32_t Example03()
{
  ...
  while (1)
  {
    ...
    if (Status != std::future_status::ready)
    {
      fmt::print("MAIN Some work in main thread...\n");
    }
    else
    {
      break;
...
int32_t Example04()
{
  ...
  std::uniform_real_distribution<> delay(500, 3000);
  for (int32_t ThreadCnt = 0; ThreadCnt < NumThreads; ThreadCnt++)
  {
    auto Wrapper = std::bind(ThreadFunctionWithArgs, delay(generator));
    std::packaged_task<int32_t()> PackagedTask(Wrapper);
...
#else
  constexpr int32_t NumThreads = 100000; // on Linux/Unix use 10000 threads
#endif
...
int32_t ThreadFunctionProducer(ppRing<ppTestObject *> *Ring, int32_t NumTestObjects)
{
  ...
  TotalProducerDuration += Duration;
  uint64_t NumTicks = EndTick - BegTick;
  TotalProducerNumTicks += NumTicks;
  return EXIT_SUCCESS;
}
...
int32_t ThreadFunctionConsumer(ppRing<ppTestObject *> *Ring)
{
  ...
  while (1)
  {
    ...
    if (Idx != Counter)
    {
      fmt::print("THRD ID={} something is wrong Idx={} Cnt={}\n", Idx, Counter);
    }
    Counter++;
  }
  ...
  uint64_t EndTick = __rdtsc();
  tDuration Duration = End - Beg;
  TotalConsumerDuration += Duration;
  uint64_t NumTicks = EndTick - BegTick;
  TotalConsumerNumTicks += NumTicks;
...
int32_t Example06()
{
  ...
  TotalProducerDuration = tDuration(0);
  TotalConsumerDuration = tDuration(0);
  int NumTestObjects = 100;

  fmt::print("MAIN Creating new thread\n");
  std::packaged_task<int32_t()> PackagedTaskProducer(std::bind(ThreadFunctionProducer, &Ring, NumTestObjects));
  ...
  double TotalProducerMilliseconds = std::chrono::duration_cast<tDurationMS>(TotalProducerDuration).count();
  double AverageProducerMilliseconds = TotalProducerMilliseconds / NumTestObjects;
  fmt::print("AverageProducerMilliseconds = {}\n", AverageProducerMilliseconds);

  double TotalConsumerMilliseconds = std::chrono::duration_cast<tDurationMS>(TotalConsumerDuration).count();
  double AverageConsumerMilliseconds = TotalConsumerMilliseconds / NumTestObjects;
  fmt::print("AverageConsumerMilliseconds = {}\n", AverageConsumerMilliseconds);

  double AverageProducerNumTicks = (double)TotalProducerNumTicks / (double)NumTestObjects;
  fmt::print("AverageProducerNumTicks = {}\n", AverageProducerNumTicks);

  double AverageConsumerNumTicks = (double)TotalConsumerNumTicks / (double)NumTestObjects;
  fmt::print("AverageConsumerNumTicks = {}\n", AverageConsumerNumTicks);

  return EXIT_SUCCESS;
}

int32_t ThreadFunctionQueueProducer(ppQueue<ppTestObject *> *Ring, int32_t NumTestObjects)
{
  std::thread::id ThreadID = std::this_thread::get_id();
  fmt::print("THRD ID={} START\n", ThreadID);
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
...
int32_t ThreadFunctionQueueConsumer(ppQueue<ppTestObject *> *Ring)
{
  std::thread::id ThreadID = std::this_thread::get_id();
  fmt::print("THRD ID={} START\n", ThreadID);
  std::this_thread::sleep_for(std::chrono::milliseconds(1000));
...
int32_t Example06q()
{
  ...
  fmt::print("MAIN Creating new thread\n");
  std::packaged_task<int32_t()> PackagedTaskProducer(std::bind(ThreadFunctionQueueProducer, &Ring, NumTestObjects));
  std::packaged_task<int32_t()> PackagedTaskConsumer(std::bind(ThreadFunctionQueueConsumer, &Ring));
  std::future<int32_t> FutureProducer = PackagedTaskProducer.get_future();
  std::future<int32_t> FutureConsumer = PackagedTaskConsumer.get_future();
  std::thread ThreadProducer = std::thread(std::move(PackagedTaskProducer));
  std::thread ThreadConsumer = std::thread(std::move(PackagedTaskConsumer));
